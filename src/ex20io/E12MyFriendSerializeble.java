...
class FriendInfoHandler {
	...
	public void addFriend(int choice) {
		...
		if(choice==1) {
			...
			//고딩친구 객체를 만둔후 컬렉션에 add한다. 인덱싱은 필요없다.
	...
	public void searchInfo() {
		...
		단, whlie문 내에서 next()를 두번 호출하면 다음..다음.. 요소가
		반환되므로 주의해야 한다.
		 */
	...
	public void deleteInfo() {
		...
		for(Friend fr : lists) {
			...
			if(deleteName.compareTo(fr.name)==0) {
				lists.remove(fr);
				deleteIndex = 1;//단순히 학제여부를 확인하기 위해 변경
				break;
			}
		}
		if(deleteIndex==-1) {
...
public class E12MyFriendSerializeble
{
